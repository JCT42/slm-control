...
class AdvancedPatternGenerator:
    def __init__(self):
        ...
        # Active area dimensions (width, height) in meters
        self.active_area = (self.width * self.pixel_pitch, self.height * self.pixel_pitch)
        
        # Initialize variables that will be set after GUI setup
        self.beam_type_var = None
        ...
        # Generate default input beam after GUI is set up
        self.input_beam = self.generate_input_beam()
        
        # Update preview to show default beam
        self.update_preview()
    ...
    def setup_gui(self):
        ...
        self.beam_type_var = tk.StringVar(value="Gaussian")  # Default beam type
    ...
    def create_controls(self):
        ...
        # Beam width control
        ttk.Label(mode_frame, text="Beam Width:").grid(row=1, column=2, padx=5, pady=5)
        beam_width_entry = ttk.Entry(mode_frame, textvariable=self.beam_width_var, width=10)
        beam_width_entry.grid(row=1, column=3, padx=5, pady=5)
        beam_width_entry.bind("<Return>", lambda e: self._on_beam_width_change())
        
        # Beam width slider
        beam_width_slider = ttk.Scale(mode_frame, from_=0.1, to=5.0, orient=tk.HORIZONTAL, length=200,
                           command=lambda v: [self.beam_width_var.set(f"{float(v):.1f}"), self._on_beam_width_change()])
        beam_width_slider.set(1.0)
        beam_width_slider.grid(row=1, column=4, padx=5, pady=5)
    ...
    def _on_beam_width_change(self, *args):
        """Handle beam width change"""
        try:
            # Update the input beam with the new width
            if self.beam_type_var.get() == "Custom" and hasattr(self, 'original_custom_image'):
                # For custom beams, reprocess with new zoom factor
                self._process_custom_beam()
            else:
                # For standard beam types, regenerate the beam
                self.input_beam = self.generate_input_beam()
            ...
            # Update status
            self.status_var.set(f"Beam width updated to: {self.beam_width_var.get()}")
        except Exception as e:
            self.status_var.set(f"Error updating beam width: {str(e)}")
            print(f"Detailed error updating beam width: {str(e)}")
            traceback.print_exc()

    def create_phase_shift_controls(self):
        """Create controls for adjusting phase shift to avoid zero-order diffraction using a blazed grating"""
        phase_shift_frame = ttk.LabelFrame(self.scrollable_frame, text="Blazed Grating Control", padding="10")
        ...
        # X-direction phase shift slider with wider range
        ...
        # Y-direction phase shift slider with wider range
        ...
        presets_frame = ttk.Frame(phase_shift_frame)
        presets_frame.grid(row=2, column=0, columnspan=4, padx=5, pady=5)
        
        ttk.Label(presets_frame, text="Presets:").pack(side=tk.LEFT, padx=5)
        
        # Add preset buttons for common shift patterns
        ttk.Button(presets_frame, text="First Order (X)", 
                 command=lambda: self.apply_preset_shift(2.0, 0.0)).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(presets_frame, text="First Order (Y)", 
                 command=lambda: self.apply_preset_shift(0.0, 2.0)).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(presets_frame, text="First Order (Diagonal)", 
                 command=lambda: self.apply_preset_shift(2.0, 2.0)).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(presets_frame, text="Reset", 
                 command=lambda: self.apply_preset_shift(0.0, 0.0)).pack(side=tk.LEFT, padx=5)
        ...
        # Help text
        help_text = "Shift your image away from the zero-order (undiffracted) light by adding a blazed grating (linear phase ramp).\n"
        help_text += "Typical values between 1.0 and 3.0 work well for shifting to the first diffraction order.\n"
        ...
        help_label.grid(row=3, column=0, columnspan=4, padx=5, pady=5)

    def apply_phase_shift(self):
        """Apply the phase shift (blazed grating) and regenerate the pattern"""
        try:
            ...
            self.status_var.set(f"Invalid shift values: {str(e)}")
        except Exception as e:
            self.status_var.set(f"Error applying blazed grating: {str(e)}")
            print(f"Detailed error: {str(e)}")
            traceback.print_exc()

    def apply_preset_shift(self, x_shift, y_shift):
        """Apply a preset shift value for the blazed grating"""
        self.phase_shift_x_var.set(f"{x_shift:.1f}")
        self.phase_shift_y_var.set(f"{y_shift:.1f}")
    ...
    def create_preview(self):
        ...
        self.ax4.set_title('Input Beam Profile')
        self.ax4.set_xticks([])
        self.ax4.set_yticks([])
    ...
    def update_preview(self):
        ...
        try:
            ...
            # Plot input beam profile - use the actual input beam used in calculations
            if hasattr(self, 'input_beam') and self.input_beam is not None:
                # Extract central region of input beam to match display size
                start_y = (self.padded_height - self.height) // 2
                end_y = start_y + self.height
                start_x = (self.padded_width - self.width) // 2
                end_x = start_x + self.width
    ...
    def load_pattern(self):
        ...
        try:
            # Use cross-platform file dialog
            ...
            # Load the pattern
            pattern = cv2.imread(filepath, cv2.IMREAD_GRAYSCALE)
            if pattern is None:
            ...
            # Generate input beam to ensure it's available for display and calculations
            self.input_beam = self.generate_input_beam()
            
            # Calculate reconstruction for preview if input beam is available
            if hasattr(self, 'input_beam') and self.input_beam is not None:
                try:
                    # Create padded phase for reconstruction
                    padded_phase = np.zeros((self.padded_height, self.padded_width))
                    start_y = (self.padded_height - self.height) // 2
                    end_y = start_y + self.height
                    start_x = (self.padded_width - self.width) // 2
                    ...
                    slm_field = self.input_beam * slm_field
                    ...
                    # This helps see details that might be lost in the high intensity regions
                    self.reconstruction = np.log1p(self.reconstruction)
                except Exception as e:
                    print(f"Error calculating reconstruction: {str(e)}")
            
            # Update all preview plots
            self.update_preview()
            self.status_var.set(f"Pattern loaded from: {filepath}")
    ...
    def save_pattern(self):
        ...
        try:
            ...
            # Save the pattern
            cv2.imwrite(filepath, self.pattern)
            self.status_var.set(f"Pattern saved to: {filepath}")
    ...
    def safe_plot_save(self, figure, default_filename=None):
        ...
        try:
            ...
            if filepath:
                ...
                self.status_var.set(f"Figure saved to {filepath}")
        except Exception as e:
    ...
    def generate_input_beam(self):
        ...
        try:
            ...
            # Calculate basic parameters - INVERTED relationship for standard beams
            # Larger beam_width value should create larger beams
            sigma_x = self.active_area[0] * beam_width / 2.355  # FWHM = 2.355 * sigma
            sigma_y = self.active_area[1] * beam_width / 2.355
            ...
            if beam_type == "Gaussian":
                ...
                radius = min(self.active_area) * beam_width / 2
            ...
            print(f"Detailed error generating input beam: {str(e)}")
            traceback.print_exc()
            # Return a default uniform beam if there's an error
    ...
    def _on_beam_type_change(self, *args):
        ...
        if self.beam_type_var.get() == "Custom":
            ...
            self.custom_input_beam = None
            # Generate and store the new input beam
            self.input_beam = self.generate_input_beam()
            # Update the preview to show the new beam
            self.update_preview()
            # Update status
    ...
    def load_custom_beam(self):
        ...
        try:
            ...
            self.original_custom_image = img.copy()
            
            # Process the custom beam with current zoom factor
            self._process_custom_beam()
            
            self.status_var.set(f"Custom beam loaded from: {filepath}")
            
        except Exception as e:
            self.status_var.set(f"Error loading custom beam: {str(e)}")
            print(f"Detailed error: {str(e)}")
            traceback.print_exc()
            # Reset to Gaussian if there was an error
            self.beam_type_var.set("Gaussian")
            self.custom_input_beam = None
            
    def _process_custom_beam(self):
        """Process the custom beam with the current zoom factor"""
        try:
            if not hasattr(self, 'original_custom_image') or self.original_custom_image is None:
                return
                
            # Get the zoom factor (same as beam width)
            zoom_factor = float(self.beam_width_var.get())
            
            # Make a copy of the original image
            img = self.original_custom_image.copy()
            
            # Create a blank padded canvas with the padded dimensions
            padded_img = np.zeros((self.padded_height, self.padded_width), dtype=np.uint8)
            
            # Calculate the aspect ratio of the input image
            img_height, img_width = img.shape
            aspect_ratio = img_width / img_height
            
            # Apply zoom factor - larger zoom factor means larger image
            # This matches the behavior of standard beam types where larger width = larger beam
            scale = zoom_factor
            
            # Calculate dimensions to fit the image within the SLM active area while preserving aspect ratio
            if aspect_ratio >= 1:  # Wider than tall
                new_width = min(int(self.width * scale), self.padded_width)
                new_height = int(new_width / aspect_ratio)
            else:  # Taller than wide
                new_height = min(int(self.height * scale), self.padded_height)
                new_width = int(new_height * aspect_ratio)
            
            # Resize the image while preserving aspect ratio
            resized_img = cv2.resize(img, (new_width, new_height))
            
            # Calculate position to center the image in the padded canvas
            start_y = (self.padded_height - new_height) // 2
            start_x = (self.padded_width - new_width) // 2
            ...
            self.custom_input_beam = padded_img.astype(float) / 255.0
            
            # Store the custom beam as the current input beam
            self.input_beam = self.custom_input_beam
            
            # Update the preview to show the custom beam
            self.update_preview()
            
        except Exception as e:
            self.status_var.set(f"Error processing custom beam: {str(e)}")
            print(f"Detailed error processing custom beam: {str(e)}")
            traceback.print_exc()
            # Return a default uniform beam if there's an error
            self.input_beam = np.ones((self.padded_height, self.padded_width))
